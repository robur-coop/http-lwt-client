...
let () =
  ...
  match Sys.argv with
  | [| _; uri |] ->
      let daemon, resolver = Http_client.Happy.stack () in
      let dns = Http_client.Mdns.create resolver in
      Http_client.Happy.inject_resolver ~getaddrinfo:(getaddrinfo dns) resolver;
      ...
      in
      let () =
        match Http_client.request ~resolver ~f ~uri acc with
        | Ok (_response, buf) ->
            pr "@[<hov>%a@]\n%!"
              (Hxd_string.pp Hxd.default)
              (Buffer.contents buf)
        | Error err -> epr "Got an error: %a\n%!" Http_client.pp_error err
      in
      Http_client.Happy.kill daemon
  | _ -> epr "%s <uri>\n%!" Sys.argv.(0)
